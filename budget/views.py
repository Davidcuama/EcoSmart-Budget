...
def remaining_budget(request):
    ...
    try:
        selected_month = int(request.GET.get('mes', current_month))
    except (TypeError, ValueError):
        selected_month = current_month

    try:
        selected_year = int(request.GET.get('anio', current_year))
    except (TypeError, ValueError):
    ...
    if not presupuestos.exists() and 'mes' not in request.GET and 'anio' not in request.GET:
        ultimo_presupuesto = Presupuesto.objects.filter(usuario=request.user).order_by('-anio', '-mes', '-id').first()
        if ultimo_presupuesto:
            selected_month = ultimo_presupuesto.mes
            selected_year = ultimo_presupuesto.anio
            presupuestos = Presupuesto.objects.filter(
                mes=selected_month,
                anio=selected_year,
                usuario=request.user,
    ...
    for presupuesto in presupuestos:
        ...
        if porcentaje >= 100:
            ...
            alertas_activas.append(
                f"{presupuesto.categoria.nombre}: excediste el presupuesto ({round(porcentaje, 2)}%)."
            )
        elif porcentaje >= 80:
            alerta = 'Preventiva'
            alerta_clase = 'warning'
            alertas_activas.append(
                f"{presupuesto.categoria.nombre}: llevas {round(porcentaje, 2)}% del presupuesto."
            )
        else:
            alerta = 'Sin alerta'
            alerta_clase = 'secondary'
    ...
    total_historico = sum(float(item['total']) for item in gastos_historicos) if gastos_historicos else 0
    ingresos_periodo = Ingreso.objects.filter(
        usuario=request.user,
        fecha__month=selected_month,
        fecha__year=selected_year,
    ).aggregate(Sum('monto'))['monto__sum'] or 0
    ...
    recomendaciones = []
    if total_historico > 0 and float(ingresos_periodo) > 0:
        for item in gastos_historicos:
            porcentaje = (float(item['total']) / total_historico) * 100
            recomendaciones.append({
                'categoria': item['categoria__nombre'],
                'promedio_historico': round(float(item['total']) / 3, 2),
                'porcentaje': round(porcentaje, 2),
                'monto_recomendado': round((float(ingresos_periodo) * porcentaje) / 100, 2),
            })
    ...
    comparacion = []
    for item in presupuestos_con_restante:
        cat = item['presupuesto'].categoria
        gasto_actual = float(item['total_gastos'])
        ...
        diferencia = gasto_actual - gasto_previo

        if gasto_previo == 0:
            tendencia     = 'Sin historial'
            tendencia_cls = 'secondary'
            cambio_pct    = None
        elif diferencia > 0:
            tendencia     = 'Subió'
            tendencia_cls = 'danger'
            cambio_pct    = round((diferencia / gasto_previo) * 100, 1)
        elif diferencia < 0:
            tendencia     = 'Bajó'
            tendencia_cls = 'success'
            cambio_pct    = round((abs(diferencia) / gasto_previo) * 100, 1)
        else:
        ...
        comparacion.append({
            ...
            'gasto_previo': round(gasto_previo, 2),
            'gasto_actual': round(gasto_actual, 2),
            'diferencia':   round(diferencia, 2),
            'cambio_pct':   cambio_pct,
            'tendencia':    tendencia,
            'tendencia_cls': tendencia_cls,
        })
    ...
    return render(request, 'budget/remaining_budget.html', {
        ...
        'periodo_historico_inicio': start_date,
        'periodo_historico_fin': end_date,
        'mes': selected_month,
        'anio': selected_year,
...
def expense_record(request):
    ...
    if request.method == 'POST':
        ...
        if not descripcion or not monto_raw:
            messages.error(request, 'Debes completar descripción y monto.')
        ...
        if categoria:
            ...
            if presupuesto and presupuesto.monto_limite > 0:
                ...
                porcentaje = (float(total_gastos) / float(presupuesto.monto_limite)) * 100
                if porcentaje >= 100:
                    messages.error(
                        request,
                        f"Alerta crítica: la categoría '{categoria.nombre}' excedió el presupuesto ({round(porcentaje, 2)}%)."
                    )
                elif porcentaje >= 80:
                    messages.warning(
                        request,
                        f"Alerta preventiva: la categoría '{categoria.nombre}' alcanzó {round(porcentaje, 2)}% del presupuesto."
                    )
            elif not presupuesto:
                messages.info(
                    request,
                    f"No hay presupuesto para '{categoria.nombre}' en {current_month}/{current_year}. "
                    "Crea uno para activar alertas y presupuesto restante."
                )

        return redirect('expense_record')

    filtro_categoria = request.GET.get('categoria', '').strip()
    gastos = Gasto.objects.filter(usuario=request.user).order_by('-fecha', '-id')
    if filtro_categoria:
...
def budget_create(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            messages.error(request, 'Mes o año inválidos.')
        ...
        return redirect(f"{reverse('remaining_budget')}?mes={mes}&anio={anio}")
